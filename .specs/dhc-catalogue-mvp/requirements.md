...
A correction (2026-09-17, task 11.6, after the 2026-09-16 rescan read a rate-limited GitHub API as six supply-chain mismatches) added 3.13: an origin the daily re-verification cannot read is recorded as not measured, fails the run by name and files no issue; the mismatch verdict is reserved for an origin that answered; the same day the read gained three retries, resting 10, 30 and 90 seconds, a primary rate limit excepted.
