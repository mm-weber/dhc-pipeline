...
A review-disposition pass (2026-09-09, D5 of `reviews/2026-09-09-requirements-vs-implementation-review.md`) amended 5.2, 5.5, 5.8, 6.1, 6.15, 6.27 and 6.37 to the mechanisms as built, where the code had settled a shape the sentence still lacked; no criterion was added or retired. Its D8 (the same day) added 6.61 and 6.62, the catalogue page: a picture rendered from the data the rescan already publishes, declined twice before the data existed and decided once it did; a follow-up the same day amended 6.47 so the published status carries each finding's decision as attested, not only its clock.
A correction (2026-09-17, task 11.6, after the 2026-09-16 rescan read a rate-limited GitHub API as six supply-chain mismatches) added 3.13: an origin the daily re-verification cannot read is recorded as not measured, fails the run by name and files no issue; the mismatch verdict is reserved for an origin that answered; the same day the read gained three retries, resting 10, 30 and 90 seconds, a primary rate limit excepted. A second correction (2026-09-20, task 11.7) added 3.14 and 3.15: GitHub's vulnerability alerts on this repository's own dependencies become a Renovate input, a fix pull request for every reported alert whatever its scope, none for a dismissed one.
...
13. IF a daily re-verification cannot read, after its retries, an origin its check needs (an upstream tag listing, GitHub's verification statement, a publisher's version statement, or a download origin's checksum sidecar) THEN THE Scan Pipeline SHALL record that definition's signal as not measured naming that origin and its HTTP status or transport error, SHALL fail that run naming it, and SHALL file no supply-chain issue for it.
14. WHEN GitHub reports a vulnerability alert on a dependency of this repository's own tooling THE Renovate Automation SHALL open a fix pull request to its lowest fixed version whatever that dependency's update scope.
15. IF a maintainer has dismissed a vulnerability alert with a reason THEN THE Renovate Automation SHALL open no fix pull request for it.
...
47. WHEN a scheduled rescan completes THE Scan Pipeline SHALL publish those times, for every finding its decision as attested (that statement's status, justification, action or impact statement and notes, with every link they carry), and, for every finding on a supported digest not yet decided or fixed, its age against that catalogue policy file's ceilings, to one status issue, as a table and as a fenced JSON block, as a workflow artifact, and to that catalogue page.
...
61. WHERE that catalogue policy file enables its catalogue page WHEN a scheduled rescan completes THE Scan Pipeline SHALL render from that run's status data, enumeration, admission proof and exception report one static page showing, per definition, its declared tags, each digest they reference with its platforms, whether that digest was scanned, attested and admitted by that verification policy that day, its findings within that decision aperture by severity with undecided and over-ceiling counts, and its lapsing exceptions, together with those clocks, that revocation record and that support statement, and SHALL publish that page and that status data to THE Repository's GitHub Pages site as a workflow artifact deployment without a commit.
62. IF that site is not enabled, that deployment fails, or that status data served after that deployment is not that run's THEN THE Scan Pipeline SHALL fail that run naming its cause.
