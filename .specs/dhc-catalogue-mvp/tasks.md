...
- [ ] 9. Production readiness, cluster A: the release path and the published set (review 2026-08-21, findings F3, F9, F6; spec amendment landed before any of these)
  - [ ] 9.1 Release arm: push by digest, scan, sign, attest, then tag
    ...
    - Recursive: `cosign sign --recursive` signs the index and every platform manifest; Syft runs once per platform manifest digest and both its SPDX and its CycloneDX output are attested to that manifest (`cosign attest --type spdxjson`, `--type cyclonedx`; CycloneDX carries the apk pull checksum that Syft's SPDX output drops, measured 2026-08-22); the single OpenVEX document is attested to the index and to each platform manifest. Measure `cosign attest --recursive` on v2.6.0 first and fall back to one `cosign attest` per manifest digest if it does not cover the children. A consumer pinning either the index or a platform digest then verifies signature, SBOM and VEX (independent review 1.8)
    ...
    - _Requirements: Req 2.2, Req 2.7, Req 2.8, Req 2.9, Req 2.10, Req 2.11, Req 2.12, Req 2.13, Req 2.26, Req 6.35, Req 6.36, Req 6.37_
  - [ ] 9.2 Daily scheduled rebuild with publish-on-change
    - `schedule:` trigger on `build.yml` (daily, before the 06:17 UTC rescan so the rescan sees fresh digests) building every definition. The `changes` job gains a schedule branch that lists every definition (its `workflow_dispatch` branch already has that shape): `github.event.before` is empty on a schedule, so the current fallback would diff `HEAD~1` and skip the release job (independent review 1.10)
  ...
  - [ ] 9.5 Legacy tags: scanned in place, never re-pointed
    ...
    - _Requirements: Req 2.6, Req 2.18, Req 2.22_
...
- [ ] 10. Production readiness, cluster B: statuses and clocks (review F5, F4, F13 i; ADR 0003, ADR 0004; spec amendment landed before any of these)
  - [ ] 10.1 Policy triage section and the exception schema
    ...
    - `triage/accepted-risk/<image>.yaml` entries gain `decided_at` (ISO date); `lint-accepted-risk.sh` requires it and checks `expired_at` minus `decided_at` against the largest ceiling, replacing the "more than 90 days from today" rule (Req 6.7, 6.11); entries are authored fresh with `decided_at` from day one, no backfill exists (greenfield epoch, task 12.1)
  ...
  - [ ] 10.2 Compiler: `affected` from exceptions, carry-forward, lapses
    ...
    - Carry-forward from the previously attested document (Req 6.37): `timestamp` kept, `last_updated` set on change (Req 6.40); a lapsed exception compiles to `under_investigation` with the original first-seen timestamp and a `status_notes` line naming the lapse, for findings the digest's report still lists (Req 6.41); no first-seen migration exists: every statement postdates the epoch (greenfield epoch, task 12.1); gate coverage ignores `affected` and `under_investigation` (Req 6.35); Trivy suppresses only `not_affected` and `fixed`, measured in design.md's supersession table and re-confirmed by the 2026-08-24 review's ordering runs
  ...
  - [ ] 10.3 Rescan re-attestation, replacing
    ...
    - No day-one repair case exists: every digest postdates the epoch and carries exactly one OpenVEX attestation from release (greenfield epoch, task 12.1); the count-repair path stays as the standing guard against the measured `--vex oci` nondeterminism (Req 6.43, 6.44; ADR 0004)
...
- [ ] 12. Greenfield epoch (owner decided 2026-08-25, F9 re-decision; executes at implementation kickoff, immediately before task 9.1's first release, after cluster D's spec)
  - [ ] 12.1 Registry wipe and the epoch record
    - [OPERATOR: delete every version of the seven packages under ghcr.io/mm-weber/dhc via the packages API; verify each repository lists zero versions] The epoch date and every removed digest recorded in `triage/LOG.md`; the ten multi-arch legacy tags and their unsigned platform manifests leave the problem space with it (critique F9, amendment 2026-08-25)
    - Open `cve` issues closed with a dated reset comment naming the epoch; the first rescan over fresh digests refiles what is real, and closed issues keep their durable markers so a returning finding reactivates rather than duplicates (Req 6.57)
    - `triage/accepted-risk/*.yaml` re-decided fresh with `decided_at` at first release; source statements under `triage/vex/` are digest-independent claims about product and version and stand unchanged; `docs/user-manual.md` and README state the epoch date where they describe the registry
    - The `/oss/release/` legacy alias matchString leaves renovate.json5: every definition is migrated, and the epoch removes its reason to exist
    - Chart image pins re-pin to the first fresh digests through the chart-pin managers in the same kickoff window, so charts reference deleted digests for at most one release cycle
    - Cluster D's `SECURITY.md` states: digests published before the epoch are withdrawn; retention is the epoch plus the declared go-forward policy
    - _Requirements: Req 6.57, Req 7.1_
...
Task group 12 (greenfield epoch) adds no new criterion: it executes the dated F9 re-decision
of 2026-08-25 and removes accommodations rather than adding standing behaviour; Req 6.57 and
7.1 are the criteria it touches.
