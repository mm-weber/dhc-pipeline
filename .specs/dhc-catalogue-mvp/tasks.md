...
- [ ] 9. Production readiness, cluster A: the release path and the published set (review 2026-08-21, findings F3, F9, F6; spec amendment landed before any of these)
  - [ ] 9.1 Release arm: push by digest, scan, sign, attest, then tag
    ...
    - Recursive: `cosign sign --recursive` signs the index and every platform manifest; Syft runs once per platform manifest digest and that SBOM is attested to that manifest; the single OpenVEX document is attested to the index and to each platform manifest. Measure `cosign attest --recursive` on v2.6.0 first and fall back to one `cosign attest` per manifest digest if it does not cover the children. A consumer pinning either the index or a platform digest then verifies signature, SBOM and VEX (independent review 1.8)
    ...
    - _Requirements: Req 2.2, Req 2.7, Req 2.8, Req 2.9, Req 2.10, Req 2.11, Req 2.12, Req 2.13, Req 2.26, Req 6.35, Req 6.36_
  - [ ] 9.2 Daily scheduled rebuild with publish-on-change
    ...
    - `scripts/package-set-diff.sh` (+ `_test.sh`) runs before 9.1's push: it takes the fresh SBOMs from the local build output and compares, per platform manifest, their (name, version) sets with the SBOMs attested to the platform manifests of the digest the same full release tag points at; equal stops the run with nothing pushed, signed or attested; different, or no published digest under that tag, publishes through 9.1 and prints the diff in the summary (Req 2.14 to 2.16)
  ...
  - [ ] 9.6 Verification policy and its daily proof
    - Verification inputs in one declared place as roles, issuer `https://token.actions.githubusercontent.com`: `releaser` (identity `https://github.com/mm-weber/dhc-pipeline/.github/workflows/build.yml@refs/heads/main`; may sign, attest `https://spdx.dev/Document`, attest `https://openvex.dev/ns`) and `re-attester` (`…/rescan.yml@refs/heads/main`; may attest `https://openvex.dev/ns` only; active from cluster B); cosign names the two predicate types `spdxjson` and `openvex`. `policies/verify-catalogue-images.yaml` renders each role as an attestor list: signature attestors are the releaser alone, and each attestation type lists the roles allowed for it (Req 2.20, 2.23). A fork adds a role as one more list entry (a dedicated signing workflow behind a protected environment, a revocation job holding `packages: write` and no `id-token`) and never widens an existing one; a `policy-controller` `ClusterImagePolicy` rendering is a fork's addition, not shipped
...
| Req 2: Image Build and Release | 3.3, 8.2, 9.1, 9.2, 9.3, 9.4, 9.5, 9.6 |
