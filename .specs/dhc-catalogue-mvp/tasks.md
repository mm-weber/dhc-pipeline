...
- [ ] 10. Production readiness, cluster B: statuses and clocks (review F5, F4, F13 i; ADR 0003, ADR 0004; spec amendment landed before any of these)
  ...
  - [ ] 10.2 Compiler: `affected` from exceptions, carry-forward, lapses
    ...
    - Carry-forward from the previously attested document (Req 6.37): `timestamp` kept, `last_updated` set on change (Req 6.40); a lapsed exception compiles to `under_investigation` with the original first-seen timestamp and a `status_notes` line naming the lapse, for findings the digest's report still lists (Req 6.41); first-seen for the twelve existing grafana exceptions, which predate any attested statement, migrates once from their `cve` issues' creation dates (2026-07-23 onward, the only recorded candidates); gate coverage ignores `affected` and `under_investigation` (Req 6.35); Trivy suppresses only `not_affected` and `fixed`, measured in design.md's supersession table and re-confirmed by the 2026-08-24 review's ordering runs
