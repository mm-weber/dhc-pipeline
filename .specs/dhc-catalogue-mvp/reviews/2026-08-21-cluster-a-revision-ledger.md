...
| 1.12 | Undefined terms: catalogue tag, full release tag, unsigned control image, signature identity in 2.10; 2.15/2.16 silent with no prior digest; which package set | **decided 2026-08-22**: catalogue tag, full release tag, platform manifest and uncovered finding defined in the Req 2 terms; the control digest named (grafana 13.0.4's amd64 child); signature identity per role (Req 2.20); the no-prior-digest case publishes (Req 2.16); the package set is the canonicalised CycloneDX projection (Req 2.15) | Req 2 terms; Req 2.15, 2.16, 2.20; task 9.6 |
| 1.13 | Design asserts as built what is only specified; old diagrams; stale 8.3 pointers; contradictory visibility bullets | **decided 2026-08-22**: security bullets marked as specified; architecture node, sequence diagram and platform header updated; 8.3 closed as superseded, its stale sentence annotated and every remaining pointer re-aimed at 9.3; visibility bullet corrected; error-table rows added for every new red-run shape | design.md, tasks.md |
...
| A5 | Req 2.21 belongs to one daily-invariants mechanism | **decided 2026-08-22**: one `invariants` step in the rescan, one report shape, the place every later cluster adds its assertion | Decision 6; task 9.4 |
...
- 2026-08-22, revision commit 8: leftovers closed (1.12, 1.13, A5). Every
  row is decided and encoded; PR #97 is ready for the owner's review. What
  the review did not see, recorded for the implementer: task 9.1 measures
  `cosign attest --recursive` on v2.6.0 and grype with an empty VEX
  document in CI; task 9.6 measures the Kyverno keyless attestor.
