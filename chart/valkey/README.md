...
| `securityContext.runAsUser/runAsGroup: 65532` | Same UID move for the containers. The rest of the restricted set (drop-ALL caps, `allowPrivilegeEscalation: false`, `readOnlyRootFilesystem: true`, `runAsNonRoot: true`, seccomp RuntimeDefault) is already upstream default and merges in — this chart arrives harder-by-default than grafana's | Req 4.3 |
