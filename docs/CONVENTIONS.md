...
| M18 | Decide what a supply-chain signal means (the rescan's `supply-chain` issue: a tag that moved, a rotated key, a compromise, a changed origin; Req 3.10) | deliberate | The rescan can measure a mismatch, not its cause; it fails daily and keeps the issue open until someone decides, while every other daily assertion and the status issue keep running (D1) | none | none |
