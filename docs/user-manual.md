...
| Go modules | `test/go.mod`, `triage/rescan/go.mod` (built-in `gomod`, `gomodTidy`); `k8s.io/**` not offered on its own, the harness's client libraries follow `sigs.k8s.io/e2e-framework` (PR #200, 2026-09-10); a GitHub vulnerability alert on any Go module, direct or indirect, opens a `[SECURITY]` fix PR outside the limits (`vulnerabilityAlerts`, Req 3.14) | go | none; reviewed |
